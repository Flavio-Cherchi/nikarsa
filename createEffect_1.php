...
//For community
    ...
    //echo "<br> questa è l'igiene " . $hygiene;
    if ($hygiene) {
        $msg .= ($hygiene < 0) ? "<br>Igiene diminuita di " . $hygiene * -1 . "." : "<br>Igiene aumentata di " . $hygiene . ".";
        $bopMsg1 .= ($hygiene < 0) ? "[*][color=#BF0000]Igiene diminuita (valore nascosto).[/color]" : "[*][color=#40BF00]Igiene aumentata (valore nascosto).[/color]";
    }

    $sql = " SELECT hygiene FROM communities WHERE ID = $ID_community AND ID_turn = $ID_turn;";
    $ris = $conn->query($sql);
    $row = $ris->fetch_assoc();
  //  $hygieneOld = $row['hygiene'];
/*
    if (($hygieneOld+$hygiene) > 100){
       $hygiene = 100 - $hygieneOld; 
    }
    */
    //echo "questa è l'igiene vecchia " . $hygieneOld;
    //echo "<br> questa è l'igiene " . $hygiene;
    

   echo $sql = " UPDATE communities SET "
...
if ($admin) {
    ...
    if ($_POST['modification']) {
        ...
        $sql = " SELECT *, communities.ID as ID_com, users.ID as ID_player, characters.ID as ID_char "
                . "FROM users "
